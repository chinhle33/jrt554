...
module.exports.run = async function ({ api, event, Threads, getText }) {
  ...
  if (typeof data["dm"] == "undefined" || data["dm"] == true) data["dm"] = false;
  else data["hi"] = true;

  await Threads.setData(threadID, { data });
  global.data.threadData.set(threadID, data);
  return api.sendMessage(`${(data["dm"] == false) ? getText("off") : getText("on")} ${getText("successText")}`, threadID, messageID);
}